...
export default function AgentLogin() {
  ...
  } = useForm<AgentLoginValues>({ resolver: zodResolver(agentLoginSchema), mode: 'onSubmit' });

  const onSubmit = async (values: AgentLoginValues) => {
    setError('');
    try {
  ...
  return (
    <ThemeScopeWrapper scope="student">
        ...
        <Card className="max-w-md w-full shadow-2xl border-none">
          <CardHeader className="text-center space-y-4">
            ...
            <div className="space-y-2">
              ...
              <CardDescription>
                Secure access for Let's Fly Together agents to manage their students.
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/30 p-4 rounded-xl text-amber-800 dark:text-amber-200 text-sm flex gap-3">
              ...
              <p>Use the credentials your administrator set up for you. All access attempts are logged.</p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
              ...
              >
                {isSubmitting ? 'Authenticating...' : 'Sign In as Agent'}
